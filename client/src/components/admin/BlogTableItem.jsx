  ...
  return (
    <tr className='border-y border-gray-300'>
      ...
      <td className='px-2 py-4 flex justify-between text-xs gap-3'>
        <button onClick={togglePublish} className={`border px-2 py-0.5 mt-1 rounded cursor-pointer mx-auto ${blog.isPublished ? 'hover:bg-orange-100/50' : 'hover:bg-green-100/50'}`}>{blog.isPublished ? 'Unpublish' : 'Publish'}</button>
        <img onClick={deleteBlog} src={assets.cross_icon} alt="cross_icon" />
      </td>
    </tr>
  )
}
